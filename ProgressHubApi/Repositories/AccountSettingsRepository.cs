...
public class AccountSettingsRepository : IAccountSettingsRepository
{
    ...
    public AccountSettingsRepository(IMongoClient client, IAccountSettingsValidator validator,
        CommonService commonService, IOptions<AvatarSettingsModel> avatarSettings)
    {
        ...
        try
        {
            ...
            if (model.File == null && user.Avatar != "")
            {
                if (!user.Avatar.Contains("google"))
                {
                    ...
                    var deleteResult = _commonService.DeleteFile(Path.Combine(Directory.GetCurrentDirectory(), "Assets",
                        "_uploads", "avatars", extractedString));
            ...
            else if (model.File != null)
            {
                if (_validator.ValidateFileForAvatar(model.File))
                {
                    ...
                    var uploadResult = _commonService.UploadFile(model.File,
                        Path.Combine(Directory.GetCurrentDirectory(), "Assets", "_uploads", "avatars"), randomPhotoName,
                        previousFileName);
            ...
            {
                user.Avatar = model.File == null
                    ? ""
                    : Environment.GetEnvironmentVariable("AVATARPATH") + randomPhotoName;
            }
            else
            {
    ...
    public async Task<PersonalDataChangeResultEnum> ChangePersonalData(PersonalDataChangeModel model)
    {
        try
        {
            var findUser = await _accounts.FindAsync(x => x.Email == model.Email);
            ...
            var validate = await _validator.ValidatePersonalData(model);
            if (validate != PersonalDataChangeResultEnum.Success) return validate;

            user.Name = model.Name;
            user.LastName = model.LastName;
            user.Nickname = model.Nickname;
            user.DateOfBirth = model.DateOfBirth == "" ? null : DateTime.Parse(model.DateOfBirth).AddDays(1);
            user.Gender = Enum.Parse<UserGenderEnum>(model.Gender);
            await _accounts.ReplaceOneAsync(x => x.Email == model.Email, user);
